...
float ftrig(float x); float flog(float x);
...
/*funcion de error*/
float ferrabs(float x, float y){ return	100*fabs((y-x)/y);}
/* funcion de falsa posicion, renglon 44*/
void falsa_posicion(float x0,float x1, int n, float (*f)(float), float(*g)(float,float))
{
float f0, f1, x2,f2, error; int vez =1;
/* calculando el valor de las funciones	*/ f0 = (*f)(x0);
f1 = (*f)(x1);
...
/* implementacion de falsa posicion */ do
{
